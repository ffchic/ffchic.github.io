...
如图所示，具体来说，在每个时间步 {% raw %}$t${% endraw %}，智能体的策略（大模型 {% raw %}$\pi${% endraw %}）会根据初始问题 {% raw %}$q${% endraw %} 和之前所有行动和观察的结果 {% raw %}$(a_1, o_1), \dots, (a_{t-1}, o_{t-1})${% endraw %}（其中 {% raw %}$(a_1, o_1)${% endraw %} 代表第一步的行动 Action 和观察 Observation），来生成当前时间步的思考 {% raw %}$th_t${% endraw %} 和行动 {% raw %}$a_t${% endraw %}：

$$
(th_t, a_t) = \pi(q, (a_1, o_1), \dots, (a_{t-1}, o_{t-1}))
$$

随后智能体中的工具 {% raw %}$\mathcal{T}${% endraw %} 会执行 {% raw %}$a_t${% endraw %}，并返回一个结果 {% raw %}$o_t${% endraw %}：
