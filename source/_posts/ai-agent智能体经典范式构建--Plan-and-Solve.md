...
我们可以将这个两阶段过程进行形式化表达。首先，规划模型 {% raw %}$\pi_{\text{plan}}${% endraw %} 根据原始问题 $q$ 生成一个包含 $n$ 个任务的计划 {% raw %}$P = (p_1, p_2, \dots, p_n)${% endraw %}：

{% raw %}
$$P = \pi_{\text{plan}}(q)$$
{% endraw %}

随后，在执行阶段，执行模型 {% raw %}$\pi_{\text{solve}}${% endraw %} 会逐一完成计划中的步骤。对于第 $i$ 个步骤，其解决方案 {% raw %}$s_i${% endraw %} 的生成会同时依赖于原始问题 $q$、完整计划 $P$ 以及之前所有步骤的执行结果 {% raw %}$(s_1, \dots, s_{i-1})${% endraw %}：
