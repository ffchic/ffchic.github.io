...
我们可以将这个两阶段过程进行形式化表达。首先，规划模型 $\pi\_{plan}$ 根据原始问题 $q$ 生成一个包含 $n$ 个任务的计划 $P = (p\_1, p\_2, p\_3, ..., p\_n)$：

$$P = \pi\_{plan}(q)$$

随后，在执行阶段，执行模型 $\pi\_{solve}$ 会逐一完成计划中的步骤。对于第 $i$ 个步骤，其解决方案 $s\_i$ 的生成会同时依赖于原始问题 $q$、完整计划 $P$ 以及之前所有步骤的执行结果 $(s\_1, ..., s\_{i-1})$：

$$s\_i = \pi\_{solve}(q, P, (s\_1, ..., s\_{i-1}))$$
