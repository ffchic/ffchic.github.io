...
我们可以将这个两阶段过程进行形式化表达。首先，规划模型 $\pi\sb{\text{plan}}$ 根据原始问题 $q$ 生成一个包含 $n$ 个任务的计划 $P = (p\sb{1}, p\sb{2}, \dots, p\sb{n})$：

$$P = \pi\sb{\text{plan}}(q)$$

随后，在执行阶段，执行模型 $\pi\sb{\text{solve}}$ 会逐一完成计划中的步骤。对于第 $i$ 个步骤，其解决方案 $s\sb{i}$ 的生成会同时依赖于原始问题 $q$、完整计划 $P$ 以及之前所有步骤的执行结果 $(s\sb{1}, \dots, s\sb{i-1})$：

$$s\sb{i} = \pi\sb{\text{solve}}(q, P, (s\sb{1}, \dots, s\sb{i-1}))$$

最终的答案就是最后一个步骤的执行结果 $s\sb{n}$。
